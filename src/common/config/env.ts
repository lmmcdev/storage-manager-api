...
export function getAllowedRoles(): string[] {
  return env.ALLOWED_ROLES.split(',')
    .map((role) => role.trim())
    .filter(Boolean);
}
...
export function validateRequiredEnv(): void {
  if (env.AUTH_MODE === 'apikey' && !env.API_KEYS) {
    throw new Error(
      'API_KEYS environment variable is required when AUTH_MODE is "apikey"'
    );
  }

  if (!env.AZURE_STORAGE_CONNECTION_STRING && !env.AZURE_STORAGE_ACCOUNT) {
    throw new Error(
      'Either AZURE_STORAGE_CONNECTION_STRING or AZURE_STORAGE_ACCOUNT must be provided'
    );
  }
}
