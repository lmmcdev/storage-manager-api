...
export function validateBody<T>(schema: z.ZodSchema<T>, body: unknown, requestId: string): T {
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      throw new BadRequestError('Validation failed', requestId, { validationErrors: details });
    }
    throw error;
  }
}
...
export function validateQuery<T>(schema: z.ZodSchema<T>, query: URLSearchParams, requestId: string): T {
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      throw new BadRequestError('Query validation failed', requestId, { validationErrors: details });
    }
    throw error;
  }
}
...
export function validateParams<T>(schema: z.ZodSchema<T>, params: Record<string, string>, requestId: string): T {
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      throw new BadRequestError('Path parameter validation failed', requestId, { validationErrors: details });
    }
    throw error;
  }
}
...
export const containerNameSchema = z.string()
  ...
  .regex(/^[a-z0-9]([a-z0-9-]*[a-z0-9])?$/, 'Container name must contain only lowercase letters, numbers, and hyphens');

export const blobNameSchema = z.string()
  .min(1, 'Blob name cannot be empty')
  .max(1024, 'Blob name must be at most 1024 characters')
  .refine(name => !name.includes('//'), 'Blob name cannot contain consecutive slashes')
